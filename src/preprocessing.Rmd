...
```{r}
df1$eq_income <- (df1$i_fihhmnnet1_dv / df1$i_ieqmoecd_dv) # 12 NAs
```
...
## Imputing duel fuel gas to electricity ratio
...
```{r}
gas_elec_xp <- !is.na(df1$i_xpgasy) & df1$i_xpgasy > 0  &
               !is.na(df1$i_xpelecy) & df1$i_xpelecy > 0 &
               !is.na(df1$i_fuelhave4) & df1$i_fuelhave4 == 0  
...
df1[gas_elec_xp,]$gas_ratio <- (df1[gas_elec_xp,]$i_xpgasy / 
                                (df1[gas_elec_xp,]$i_xpelecy + df1[gas_elec_xp,]$i_xpgasy))
df1[gas_elec_xp,]$elec_ratio <- 1 - df1[gas_elec_xp,]$gas_ratio
```
...
dfimp <- df1[!is.na(df1$i_fuelduel),] %>% select(elec_ratio,i_fihhmnnet1_dv,i_hhsize, i_nkids_dv,i_nonepar_dv,i_ncouple_dv,i_npensioner,i_nue_dv,nfem,nhcond,i_hsbeds,i_hsrooms,i_hsownd,hh_hiqual,i_gor_dv,i_urban_dv, i_fborn,i_fuelhave1, i_fuelhave2, i_fuelhave3,i_heatch)
dfimp <- dfimp[,2:ncol(dfimp)]

dfimp[1:12] <- sapply(dfimp[1:12], haven::zap_labels) 
dfimp[13:ncol(dfimp)] <- sapply(dfimp[13:ncol(dfimp)], haven::as_factor) 
...
var(df1$elec_ratio, na.rm=T)

mean(imp$imp$elec_ratio[,1], na.rm=T)
mean(df1$elec_ratio, na.rm=T)


df1[!is.na(df1$i_fuelduel),]$elec_ratio <- complete(imp)$elec_ratio
...
```{r}
# df1[hh_duelxp & hh_no_oil,]$elec_ratio <- svymean(~elec_ratio, subset(svy_df, gas_elec_xp & hh_no_oil))[1]
# df1[hh_duelxp & hh_no_oil,]$gas_ratio  <- svymean(~gas_ratio , subset(svy_df, gas_elec_xp & hh_no_oil))[1]
# df1[hh_duelxp & hh_oilhav,]$elec_ratio <- svymean(~elec_ratio, subset(svy_df, gas_elec_xp & hh_oilhav))[1]
# df1[hh_duelxp & hh_oilhav,]$gas_ratio  <- svymean(~gas_ratio , subset(svy_df, gas_elec_xp & hh_oilhav))[1]
# 
...
```{r}
svytotal(~I(i_hhdenui_xw > -1), subset(svy_df, hh_duelxp))
```
... and for HH for which the ratio is available in the data.
```{r}
svytotal(~I(i_hhdenui_xw > -1), subset(svy_df, gas_elec_xp))
```
The electricity / gas ratio for HH without oil is
```{r}
df1[hh_duelxp,]$elec_ratio[1] / df1[hh_duelxp,]$gas_ratio[1]
df1[gas_elec_xp,]$elec_ratio[1] / df1[gas_elec_xp,]$gas_ratio[1]
```

```{r}
df1[hh_duelxp & hh_oilhav,]$elec_ratio[1] / df1[hh_duelxp & hh_oilhav,]$gas_ratio[1]
```
...
sum(df1$i_xpgasy > 0 | df1$i_xpelecy > 0 | 
    df1$i_xpoily > 0 | df1$i_xpduely > 0, na.rm=T) == sum(df1$total_co2 > 0, na.rm=T)
```
...
```{r}
unknown_fuels = !is.na(df1$i_fuelhave4) & df1$i_fuelhave4 == 1
```
...
```{r}
known_people = !(unknown_fuels | unknown_elec | unknown_gas | unknown_oil | 
                unknown_co2)
```
...
S_cont <- svyby(~ eq_income / i_hhsize / i_nkids_dv / i_hsbeds / i_fuelhave3, ~hh_duelxp, svy_all, svymean, na.rm = T)
# Initialize table
...
### Other fuels
We did not consider emissions from other fuels in future work. The expenditure on those fuels is comparatively small. Nevertheless, it might be interesting to study those in future work.
```{r}
...
rowform = ~eq_income / i_hhsize / i_nkids_dv / i_hsbeds / i_fuelhave3 
