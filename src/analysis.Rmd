...
```
```{r}
svy_good_per_capita <- svydesign(
  id=~i_psu, strata=~i_strata, weights=~I(i_hhdenui_xw * i_hhsize), data=svy_good$variables)
svy_all_per_capita <- svydesign(
  id=~i_psu, strata=~i_strata, weights=~I(i_hhdenui_xw * i_hhsize), data=svy_all$variables)
```
...
per_capita_co2_quartiles <- svyquantile(~per_capita_co2, svy_good_per_capita, seq(from=0.25, to=0.75, by=0.25))

my_breaks <- svyhist(~per_capita_co2, svy_good_per_capita, breaks=nbreaks)$breaks
...
svyhist(~per_capita_co2, svy_good_per_capita, 
        breaks=my_breaks, 
        col=my_colors,
        xlab = "",
        main = "CO2 in kg",
...
pc_null_model <- svyglm(per_capita_co2 ~ 1, svy_good_per_capita, na.exclude(TRUE), family = gaussian()) 
...
svyglm_extended_pc <- svyglm(per_capita_co2 ~
                                ...
                                as.factor(i_fuelhave3) +
                                i_heatch,
                                svy_good_per_capita, na.exclude(TRUE), family = gaussian()) 
...
```{r}
pc_income_deciles <- svyquantile(~I(i_fihhmnnet1_dv/i_hhsize), svy_all_per_capita, seq(from=0.1, to=0.9, by=0.1))
svy_good_per_capita <- update(svy_good_per_capita, pc_income = i_fihhmnnet1_dv/i_hhsize)
svy_good_per_capita <- update(svy_good_per_capita, 
                  pc_income_decile = factor(1 + findInterval(pc_income, pc_income_deciles)))

eq_income_deciles <- svyquantile(~eq_income, svy_all_per_capita, seq(from=0.1, to=0.9, by=0.1))
svy_all_per_capita <- update(svy_all_per_capita, 
...
# co2_vs_income_decile = svyby(~per_capita_co2 / total_co2, # WEIGHTS CORRECT?
#                              ~eq_income_decile, svy_good, svymean)
# eq_income_vs_income_decile = svyby(~eq_income, 
#                              ~eq_income_decile, svy_good, svymean)

pc_co2_vs_eq_income_decile = svyby(~per_capita_co2,
                             ~eq_income_decile, svy_good_per_capita, svymean)
hh_co2_vs_eq_income_decile = svyby(~total_co2,
                             ~eq_income_decile, svy_good, svymean)
eq_income_vs_income_decile = svyby(~eq_income, 
                             ~eq_income_decile, svy_good_per_capita, svymean)
...
co2_vs_pc_income_decile = svyby(~per_capita_co2, 
                             ~pc_income_decile, svy_good_per_capita, svymean)
...
```{r}
pdf("income_vs_co2_newest.pdf", 14, 7)
...
plot(c(eq_income_vs_income_decile$eq_income, hh_income_vs_hh_income_decile$i_fihhmnnet1_dv),
     c(hh_co2_vs_eq_income_decile[,2],co2_vs_hh_income_decile[,2]),
     xlab= "Monthly Income", 
     ylab="Household CO2 Emissions in kg",
     ylim = c(0,8000),
     xlim = c(0,8000),
...
#axis(1, at=1:10, labels = round(eq_income_vs_income_decile$eq_income), las = 2)
lines(eq_income_vs_income_decile$eq_income, hh_co2_vs_eq_income_decile[,2],
      col = col_eq)
...
plot(c(eq_income_vs_income_decile$eq_income,pc_income_vs_pc_income_decile$pc_income),
     c(pc_co2_vs_eq_income_decile[,2], co2_vs_pc_income_decile[,2]),
     xlab= "Monthly Income", 
     ylab="Per Capita CO2 Emissions in kg",
     ylim = c(0,4000),
     xlin = c(0,4000),
...
lines(eq_income_vs_income_decile$eq_income, pc_co2_vs_eq_income_decile[,2],
      col = col_eq)

abline(h = svymean(~per_capita_co2, svy_good_per_capita),
       col = col_eq, lty = 3)
