...
```{r}
rn = c("HH CO2 in kg","PC CO2 in kg", "eq. income in POUNDS", "# persons", "# kids")
cn = c("Owned outright", 
       "Owned on mortgage",
       "Shared ownership", 
       "Rented", 
...
S_cont <- svyby(~total_co2 / per_capita_co2 / eq_income / i_hhsize / i_nkids_dv, 
                ~i_hsownd, svy_good, svymean)

tab <- sprintf("%i (%i)", round(S_cont[,2]), round(S_cont[,7]))
for (i in 3:4){
  tab <- cbind(tab, sprintf("%i (%i)", round(S_cont[,i]), round(S_cont[,i+5])))
}
tab <- cbind(tab, sprintf("%.2f (%.2f)", S_cont[,5], S_cont[,5+5]))
tab <- cbind(tab, sprintf("%.2f (%.2f)", S_cont[,6], S_cont[,6+5]))

ptab <- t(matrix(t(tab),length(rn), 5,dimnames=list(rn,cn)))
ptab
```
```{r eval=FALSE}
xtable(ptab,
       size = "\\small",
       caption = "Mean values (standard deviations) of household (HH) and per capita (PC) annual CO2 emissions in kg, equivalised annual household income in \\pounds, household size and number of kids with respect to ownership status of accomodation. On mortgage could be either owned or being bought. \\label{tab:ownership}")
```
...
Household quartiles
```{r}
hhtab <- household_table(~total_co2_quartile)
hhtab
```

Per-Captia quartiles
```{r}
pctab <- household_table(~per_capita_co2_quartile)
pctab
```
...
```{r}
hhtab <- housing_table(~total_co2_quartile)
hhtab
```

Per Capita quartiles
```{r}
pctab <- housing_table(~per_capita_co2_quartile)
pctab
```
