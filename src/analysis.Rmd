...
```{r}
rn = c("HH CO2 in kg","PC CO2 in kg", "eq. income in POUNDS", "# persons", "# kids", "# bedrooms")
cn = c("Owned outright", 
       "Owned on mortgage",
       "Shared ownership", 
       "Rented", 
...
S_cont <- svyby(~total_co2 / per_capita_co2 / eq_income / i_hhsize / i_nkids_dv / i_hsbeds,
                ~i_hsownd, svy_good, svymean, na.rm=T)

tab <- sprintf("%i (%i)", round(S_cont[,2]), round(S_cont[,8]))
for (i in 3:4){
  tab <- cbind(tab, sprintf("%i (%i)", round(S_cont[,i]), round(S_cont[,i+6])))
}
tab <- cbind(tab, sprintf("%.2f (%.2f)", S_cont[,5], S_cont[,5+6]))
tab <- cbind(tab, sprintf("%.2f (%.2f)", S_cont[,6], S_cont[,6+6]))
tab <- cbind(tab, sprintf("%.2f (%.2f)", S_cont[,7], S_cont[,7+6]))
...
xtable(ptab,
       size = "\\small",
       caption = "Mean values (standard deviations) of household (HH) and per capita (PC) annual CO2 emissions in kg, equivalised annual household income in \\pounds, household size, number of kids and number of bedrooms with respect to ownership status of accomodation. \\label{tab:ownership}")
```
